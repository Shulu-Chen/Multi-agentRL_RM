...
'''
 @Time    : 2021/7/5 14:52
 @Author  : Shulu Chen
 @FileName: environment_manual.py
 @Software: PyCharm
'''
import gym
...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        self.low_count=[0,0]
        self.high_count=[0,0]
        self.EMRSb=15+st.norm.ppf(1-187.5/337.5)*5
        self.high_seats0=[]
        self.high_seats1=[]
        self.low_seats0=[]
        self.low_seats1=[]
        ...
        self.observation_space.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(4,), dtype=np.float32))
        self.observation_space.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(4,), dtype=np.float32))
    ...
    def step(self, action_n,pax,left_Seats,day):
        ...
        agent_action_h=[]
        agent_action_l=[]
        # print(action_n[0],"action 0")
        for i, agent in enumerate(self.agents):
             self._set_action(action_n[i], agent, self.action_space[i])
             agent_action_h.append(agent.action.u[0])
             agent_action_l.append(agent.action.u[1])
        # print(agent_action_h,"selected high price")
        # print(agent_action_l,"selected low price")
        if day<=10:
            agent_action_h[1]=2.75*2
            agent_action_l[1]=1.25
        elif day>10 and day<=20:
            agent_action_h[1]=3.25*2
            agent_action_l[1]=1.75
        elif day>20 and day<=30:
            agent_action_h[1]=3.75*2
            agent_action_l[1]=2.25
        elif day>30:
            agent_action_h[1]=4.25*2
            agent_action_l[1]=2.75
        done_list=[]
    ...
    def reset(self):
        ...
        #     obs_n.append(self._get_obs(agent))
        #TODO: size of obs
        obs_n.append(np.array([0,0,50,0]))
        obs_n.append(np.array([0,0,50,0]))
        return obs_n
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        action_choose_h=[i*0.02 for i in [250,275,300,325,350,375,400,425]]
        action_choose_l=[i*0.01 for i in [100,125,150,175,200,225,250,275]]

        action_h=action[0].tolist()
        action_l=action[1].tolist()
        #TODO: max of action output?
        # print(action_h,"action_h")
    ...
    def Test_model(self,episode_step,plot_graph,price_curve):
        ...
        if plot_graph:
            ...
            plt.axhline(y=50-self.EMRSb, label="EMSRb-low",color="green", linestyle='-.')
        ...
        if price_curve:
            ...
            plt.legend()
            plt.show()
            print(self.price[0],"Agent0 High Class")
            print(self.price[1],"Agent1 High Class")
            print(self.price[2],"Agent0 low Class")
            print(self.price[3],"Agent1 low Class")
            print(self.high_seats0,"0 H")
            print(self.high_seats1,"1 H")
            print(self.low_seats1,"1 l")
            print(self.low_seats0,"0 l")
            print(self.reward_list[0][-1],"reward 0")
            print(self.reward_list[1][-1],"reward 1")
