...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        # create action distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]

        # set up placeholders for observation and actions
        ...
        #train is the loss of training network
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents, t):
        ...
        if len(self.replay_buffer) < self.max_replay_buffer_len: # replay buffer is not large enough
            ...
            return
        if not t % 100 == 0:  # only update every 100 steps#TODO:soft update
            return
        print("update")
        ...
        for i in range(num_sample):
            ...
            target_q_next = self.q_debug['target_q_values'](*(obs_next_n + target_act_next_n))   #TODO
